...
//
// Требование краткости здесь же и не случайно: каждый ответ целиком уезжает обратно
// в контекст следующего запроса, поэтому лишний абзац оплачивается не один раз,
// а на каждом последующем ходе диалога.
const formatRules = "Отвечай на русском языке в простом Markdown. " +
	...
	"Не используй LaTeX. " +
	"Отвечай максимально коротко и лаконично: только суть, без вступлений, " +
	"без повторов вопроса и без заключительных пересказов сказанного. " +
	"По умолчанию укладывайся в несколько предложений или короткий список; " +
	"разворачивай ответ подробно, только если пользователь прямо об этом попросил. " +
	"Если хватает одного предложения -- отвечай одним предложением."
...
var Presets = []Preset{
	{
		...
		Prompt: "Ты — полезный ассистент. " + formatRules,
	},
	{
		ID:    "stepwise",
		Title: "Пошагово",
	...
	},
	{
		...
		Prompt: "Ты — аналитик. Начни с разбора условия: выпиши все данные, явные и неявные " +
			...
			"Только после разбора дай решение и итоговый ответ. " + formatRules,
	},
	{
		ID:    "engineer",
		Title: "Инженер",
