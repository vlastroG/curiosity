...
var routerSystem = fmt.Sprintf(
	"Ты — диспетчер помощника строителя. Ты не разговариваешь с пользователем: "+
		...
		"ПРОФИЛЬ. Если ниже приведён профиль пользователя, он действует и на \"start\", "+
		"и на \"collect\". Пункт, ответ на который в профиле уже есть, сразу клади "+
		"в answers со значением из профиля и припиской \"(из профиля)\" — спрашивать "+
		"человека о том, что он уже про себя написал, нельзя. Заполняй только те пункты, "+
		"на вопрос которых профиль отвечает прямо; требования к самому плану "+
		"(\"не предлагай...\", \"без спецтехники\") ответами не считаются и в answers не идут.\n\n"+
...
func routerPayload(in RunInput, question string) string {
	...
	if facts := in.Profile.Facts(); facts != "" {
		payload.WriteString("\n")
		payload.WriteString(facts)
	}
