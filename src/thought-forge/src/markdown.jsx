...
/**
 * Промпты просят закончить ответ строкой "Ответ: ...". Вытаскиваем её, чтобы
 * показать итог отдельно -- в сравнении четырёх колонок это главное, что нужно
 * увидеть сразу, не вчитываясь в рассуждение.
 */
const VERDICT = /^\s*\**\s*Ответ\s*:?\**\s*(.*)$/i;
...
export function splitVerdict(text) {
  ...
  for (let i = lines.length - 1; i >= 0 && i >= lines.length - 4; i -= 1) {
    const match = lines[i].match(VERDICT);
    if (match && match[1].trim() !== '') {
      return {
        verdict: match[1].replace(/\*\*/g, '').trim(),
