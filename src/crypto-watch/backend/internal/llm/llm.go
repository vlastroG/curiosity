// Package llm -- тонкий клиент к OpenAI-совместимым чат-API с вызовом инструментов.
//
// DeepSeek и OpenRouter говорят на одном протоколе, поэтому провайдер -- это данные
// (адрес, ключ, пара заголовков), а не отдельный код на каждого. Пакет ничего не
// знает ни про крипту, ни про MCP: отправить диалог, получить ответ или заявки
...
// New создаёт клиент с таймаутом на один запрос.
func New(timeout time.Duration) *Client {
	return &Client{http: &http.Client{Timeout: timeout}, backoff: 3 * time.Second}
}
...
func (c *Client) once(ctx context.Context, p Provider, body []byte) (Response, bool, error) {
