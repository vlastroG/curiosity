...
//
// Достраивать приходится бюджет вывода при смене модели. Правило живёт здесь,
// а не в HTTP-слое, чтобы его нельзя было обойти и чтобы проверка видела уже
// пересчитанное значение: у бесплатной модели потолок вывода в сорок раз ниже,
// чем у DeepSeek, и бюджет с платной её не прошёл бы.
func (c *Chat) ApplyConfig(cfg agent.Config) error {
	if cfg.Model != c.Config.Model {
		cfg.MaxTokens = agent.MaxTokensAfterSwitch(c.Config.Model, cfg.Model, cfg.MaxTokens)
	}
